...
function ReviewQueuePanel({ userEmail }) {
  ...
  const [resolving, setResolving] = React.useState(null); // reviewId being resolved
  const [denyId, setDenyId]       = React.useState(null); // reviewId for deny note modal
  ...
  function ReviewCard({ review, isResolved }) {
    ...
    return (
      <div style={{
        background: 'var(--bg-elevated)', border: `1px solid ${isResolved ? 'var(--border)' : 'rgba(255,184,0,0.2)'}`,
        borderRadius: '10px', overflow: 'hidden',
        opacity: isProcessing ? 0.7 : 1, transition: 'opacity 0.2s',
      }}>
        {/* Header row */}
        <div style={{ padding: '8px 12px', background: isResolved ? 'rgba(255,255,255,0.02)' : 'rgba(255,184,0,0.04)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {isResolved && (
            <span style={{ fontSize: '0.65rem', fontWeight: 700, color: review.status === 'APPROVED' ? '#4ade80' : '#f87171', background: review.status === 'APPROVED' ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)', borderRadius: '4px', padding: '1px 6px' }}>{review.status}</span>
          )}
        </div>
        ...
        <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: 1.5, background: 'rgba(255,255,255,0.03)', borderRadius: '6px', padding: '6px 9px', borderLeft: '3px solid rgba(96,165,250,0.4)' }}>
              {review.user_message}
            </div>
          </div>
          ...
          {review.ai_analysis && (
            <div>
              ...
              <div style={{ fontSize: '0.76rem', color: '#ccc', lineHeight: 1.5, background: 'rgba(255,184,0,0.04)', borderRadius: '6px', padding: '6px 9px', borderLeft: '3px solid rgba(255,184,0,0.3)' }}>
                {review.ai_analysis}
              </div>
            </div>
          )}
          ...
          {isResolved && review.admin_notes && (
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontStyle: 'italic', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '6px' }}>
              Admin note: {review.admin_notes}
            </div>
          )}
        </div>
        ...
        {!isResolved && (
          <div style={{ padding: '8px 12px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '8px', alignItems: 'center', background: 'rgba(0,0,0,0.1)' }}>
            <button
              ...
              style={{ padding: '5px 14px', borderRadius: '7px', border: 'none', background: isProcessing ? '#333' : '#4ade80', color: '#000', cursor: isProcessing ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.78rem' }}
            >{isProcessing ? '…' : `✓ Approve${hasChanges ? ' & Apply' : ''}`}</button>
            <button
              onClick={() => { setDenyId(review.id); setDenyNote(''); }}
              disabled={isProcessing}
              style={{ padding: '5px 14px', borderRadius: '7px', border: '1px solid rgba(248,113,113,0.3)', background: 'transparent', color: '#f87171', cursor: isProcessing ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.78rem' }}
            >✗ Deny</button>
            {!hasChanges && (
              <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>No changes extracted — approving will log the review only</span>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div>
      {/* Deny note modal */}
      {denyId !== null && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 9500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
          onClick={e => { if (e.target === e.currentTarget) { setDenyId(null); setDenyNote(''); } }}>
          <div style={{ background: 'var(--bg-surface)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: '14px', padding: '1.25rem', width: '100%', maxWidth: '380px', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
            ...
            <textarea
              ...
              placeholder="Optional: explain why the request was denied…"
              rows={3}
              maxLength={500}
              style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '7px', padding: '0.5rem 0.7rem', color: 'var(--text-primary)', fontSize: '0.8rem', resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.5 }}
            />
      ...
      {/* Status bar */}
      {actionMsg && <div style={{ padding: '8px 12px', borderRadius: '7px', background: actionMsg.startsWith('✓') ? 'rgba(74,222,128,0.08)' : actionMsg.startsWith('✗') ? 'rgba(248,113,113,0.06)' : 'rgba(248,113,113,0.06)', color: actionMsg.startsWith('✓') ? '#4ade80' : '#f87171', fontSize: '0.8rem', marginBottom: '0.75rem', border: `1px solid ${actionMsg.startsWith('✓') ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'}` }}>{actionMsg}</div>}
...
CREATE INDEX ON pick_review_requests (status, created_at DESC);`}</pre>
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
        <button
          onClick={() => {
            setShowResolved(v => !v);
            if (!showResolved && resolved.length === 0) loadResolved();
          }}
          style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.8rem', padding: 0, display: 'flex', alignItems: 'center', gap: '6px' }}
        >
