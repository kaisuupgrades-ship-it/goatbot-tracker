...
// Stadium DB + INDOOR_SPORTS now live in @/lib/stadiums so server enrichment
// (api/sports?enrich=1) and client rendering share one source of truth.
...
export function GameCard({ event, sport, onAnalyze, onAddBet, starred, onStar, injuries, injuriesChecked, isAllMode, suppressHeader = false, externalExpanded = null, oddsFormat = 'american', timezone = 'America/New_York', gameLeans = {}, parlayMode = false, parlayLegs = [], onAddParlayLeg }) {
  ...
  // Seed H2H from (1) server-side enrichment baked into event.enrichment.h2h
  // by /api/sports?enrich=1 — instant; (2) module-level cache from a prior
  // hover/expand; (3) null otherwise. Server-baked data also pre-warms the
  // module cache so other cards with the same matchup get it for free.
  const h2hKey = `${sport}_${home.team?.id}_${away.team?.id}`;
  const [h2hData,  setH2hData]  = useState(() => {
    const baked = event?.enrichment?.h2h;
    if (baked && baked.record) {
      // Warm the shared cache so siblings + hover-prefetch skip the network.
      try { h2hCache.set(h2hKey, { data: baked, time: Date.now(), ok: true }); } catch {}
      return baked;
    }
  ...
  // Eager prefetch + cache seeding: if /api/sports?enrich=1 baked H2H/weather
  // into this event, write them into the module-level caches immediately so
  // WeatherWidget and any sibling card with the same matchup render with no
  // network. If no baked data, fall through to the on-mount prefetch (warms
  // before the user expands).
  useEffect(() => {
    const bakedH2H = event?.enrichment?.h2h;
    if (bakedH2H && bakedH2H.record && awayTeamId && homeTeamId) {
      try { h2hCache.set(h2hKey, { data: bakedH2H, time: Date.now(), ok: true }); } catch {}
    } else {
      prefetchH2H();
    }

    const bakedWx = event?.enrichment?.weather;
    if (bakedWx && stadium?.lat) {
      const wKey = `${stadium.lat}_${stadium.lon}_${event.date || ''}`;
      try { weatherCache.set(wKey, { data: bakedWx, time: Date.now() }); } catch {}
    } else if (stadium?.lat && !isIndoorSport && !stadium.dome) {
      ...
      if (s === 'all') {
        ...
        const results = await Promise.allSettled(
          ALL_SPORTS_KEYS.map(key =>
            fetch(`/api/sports?sport=${key}&endpoint=scoreboard&date=${espnDate}&enrich=1`)
              .then(r => r.json())
        ...
        const res = await fetch(`/api/sports?sport=${s}&endpoint=scoreboard&date=${espnDate}&enrich=1`);
