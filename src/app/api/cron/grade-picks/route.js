...
export async function GET(req) {
  ...
  // Post-mortems are now generated by the betos-postmortem-catchup scheduled
  // task (runs daily under Pro Max — zero API tokens). The inline path stays
  // gated behind a settings flag so it can be flipped back on as a fallback
  // without a redeploy.
  let postMortemStats = { generated: 0, skipped: 0, errors: 0, disabled: false };
  const { data: pmSetting } = await supabase
    .from('settings').select('value').eq('key', 'cron_post_mortems_enabled').maybeSingle();
  const inlinePostMortemsEnabled = pmSetting?.value === 'true' || pmSetting?.value === true;
  if (inlinePostMortemsEnabled) {
    try {
